...
⚠️ **Importante:** Não recomendo substituir o comando `ls` padrão (ex: `alias ls="eza"`). Com ferramentas de IA como GitHub Copilot e Claude, os agentes precisam do output padrão do `ls` para entender a estrutura de diretórios. Crie aliases separados como `lz` e `lsz` para usar quando precisar da visualização com ícones.
...
⚠️ **Importante:** Use o comando `z` ao invés de substituir o `cd` padrão (ex: `alias cd="z"`). Substituir o `cd` pode afetar agentes de IA e o autocompletar do shell. Quanto mais você usar o comando `z` para navegar, mais o histórico será atualizado, permitindo pular para pastas mais facilmente.
